...
/* Create and log a variable `twelveString` that is the result of passing "8" to 
   your addFour() function. Consider what this tells you about how the function 
   should be described (e.g., in a comment). */
...
/* Define a function `fizzBuzz` that takes in a single number as an argument.
   The function should return an array of numbers from 1 to the argument. But 
   for multiples of three, the array should contain "Fizz" instead of the number.
   For multiples of five, the array should contain "Buzz" instead of the number. 
   For numbers which are multiples of both three and five, the array should 
   contain "FizzBuzz" instead of the number.
   The returned array should be empty for arguments less than 1. */

   

/* Define a function `getLetterFrequencies()` that takes in a single string as an
   argument. The function should *return* an Object whose keys are characters
   ...
   time (you can access them with bracket notation, like an array). For each
   letter, increase the value associated with that key by one. Watch out for if
   the letter is not in the Object yet!
...
/* Create a variable `deck` that represents a deck of modern playing cards
   ...
   *array* of 52 elements, each of which is an Object with properties:
    ...
    Log out the `deck` to check your work! */

    

//You can test the below functions by creating a `pokerHand` array that contains
...
/* Define a function `getHighCard()` that takes in an array of "card" objects
  ...
  with the highest rank. Cards of different suits but the same rank are considered
...
/* Bonus challenge: define a function `hasPair()` that takes in an array of 
   "card" objects and returns whether or not there is at least one _pair_ (two 
   cards with the same _rank_) in the array.
   Double bonus: return the rank of the pair of cards with the highest rank 
